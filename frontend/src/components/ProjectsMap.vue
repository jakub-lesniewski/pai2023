<template>
  <div v-if="markers.length > 0">
    <v-select
      ...
      v-model="selected"
      :items="
        markers.map((item, index) => ({ value: index, title: item.title }))
      "
    ...
    <div v-if="markers.length == 0">No projects</div>
...
export default {
  ...
  methods: {
    selectProject() {
      this.center = {
        lat: this.markers[this.selected].lat,
        lng: this.markers[this.selected].lng,
      };
